...
import { Search, ChevronRight, Zap, ArrowLeft, BookMarked, FileCode2 } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import { fastapiExamples, FastAPIExample } from "@/data/fastapi-examples";
import { CodeBlock } from "@/components/CodeBlock";

export default function FastAPIExamplesPage() {
  const [searchQuery, setSearchQuery] = useState("");
  const [selectedId, setSelectedId] = useState<string | null>(null);

  const filtered = useMemo(() => {
    if (!searchQuery.trim()) return fastapiExamples;
    const q = searchQuery.toLowerCase();
    return fastapiExamples.filter(
      (e) =>
  ...
  const selected = useMemo(
    () => fastapiExamples.find((e) => e.id === selectedId) ?? null,
    [selectedId]
  );
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] max-w-7xl mx-auto overflow-hidden bg-background">
        ...
        <div className="p-4 border-b border-border/50 bg-background/50 backdrop-blur z-10 sticky top-0">
          <div className="relative group">
            ...
            <input
              ...
              placeholder="Поиск задач FastAPI..."
              value={searchQuery}
      ...
      >
        {selected ? (
          <ExampleDetail example={selected} onBack={() => setSelectedId(null)} />
        ) : (
          <EmptyState />
        )}
      </div>
  ...
  return (
    <div className="flex-1 flex items-center justify-center flex-col text-center p-8 h-full">
      <div className="w-20 h-20 bg-muted rounded-3xl flex items-center justify-center mb-6 shadow-inner rotate-3">
        <Zap className="w-10 h-10 text-muted-foreground -rotate-3" />
      </div>
      <h2 className="text-2xl font-bold text-foreground mb-2">Сложные примеры FastAPI</h2>
