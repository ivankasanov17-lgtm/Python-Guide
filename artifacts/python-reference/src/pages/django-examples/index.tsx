...
import { djangoExamples, DjangoExample } from "@/data/django-examples";
import { CodeBlock } from "@/components/CodeBlock";

export default function DjangoExamplesPage() {
  const [searchQuery, setSearchQuery] = useState("");
  const [selectedId, setSelectedId] = useState<string | null>(null);

  const filtered = useMemo(() => {
    if (!searchQuery.trim()) return djangoExamples;
    const q = searchQuery.toLowerCase();
    return djangoExamples.filter(
      (e) =>
  ...
  const selected = useMemo(
    () => djangoExamples.find((e) => e.id === selectedId) ?? null,
    [selectedId]
  );
        ...
        <div className="p-4 border-b border-border/50 bg-background/50 backdrop-blur z-10 sticky top-0">
          <div className="relative group">
            ...
            <input
              ...
              placeholder="Поиск задач Django..."
              value={searchQuery}
...
function EmptyState() {
  return (
    <div className="flex-1 flex items-center justify-center flex-col text-center p-8 h-full">
      ...
      <h2 className="text-2xl font-bold text-foreground mb-2">Сложные примеры Django</h2>
